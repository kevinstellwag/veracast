...
export async function POST(req: NextRequest) {
  ...
  switch (action) {
    ...
    case 'verify_user': {
      await supabaseAdmin.from('users').update({ is_verified: true }).eq('id', targetId)
      return NextResponse.json({ data: { ok: true, action: 'user_verified' } })
    }

    case 'unverify_user': {
      await supabaseAdmin.from('users').update({ is_verified: false }).eq('id', targetId)
      return NextResponse.json({ data: { ok: true, action: 'user_unverified' } })
    }
